class mx.video.VideoPlayer extends MovieClip
{
        ...
        switch (info.code)
        {
            case "NetStream.Play.Stop":
            {
                ...
                switch (_state)
                {
                    ...
                    default:
                    {
                        if (_bufferState == mx.video.VideoPlayer.BUFFER_EMPTY || _bufferTime <= 1.000000E-001)
                        {
                        ...
                        else if (_bufferState == mx.video.VideoPlayer.BUFFER_FULL)
                        {
                            _bufferState = mx.video.VideoPlayer.BUFFER_FULL_SAW_PLAY_STOP;
    ...
    function doAutoResize()
    {
        if (_autoResizeIntervalID > 0)
        {
            ...
            if (_video.width != _prevVideoWidth || _video.height != _prevVideoHeight || _bufferState >= mx.video.VideoPlayer.BUFFER_FULL || _ns.time > mx.video.VideoPlayer.AUTO_RESIZE_PLAYHEAD_TIMEOUT)
            {
