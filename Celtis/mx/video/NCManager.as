...
{
    var _timeoutIntervalId, _tryNCIntervalId, _timeout, _isRTMP, _serverName, _wrappedURL, _portNumber, _appName, _contentPath, _streamName, _streamLength, _streamWidth, _streamHeight, _streams, _autoSenseBW, _payload, _connTypeCounter, _bitrate, _owner, _nc, _protocol, _smilMgr, mc, _ncUri, fallbackServerName, _tryNC;
    function NCManager()
    {
        this.initNCInfo();
        this.initOtherInfo();
    ...
    function connectToURL(url)
    {
        ...
        else if (_loc2.streamName.slice(-4).toLowerCase() == ".flv")
        {
    ...
    function helperDone(helper, success)
    {
        ...
        var _loc2;
        var _loc4 = _smilMgr.baseURLAttr[0];
    ...
    function bitrateMatch()
    {
        ...
        else
        {
            ...
            {
                if (isNaN(_streams[_loc2].bitrate) || _bitrate <= _streams[_loc2].bitrate)
                {
                    _loc3 = _loc2;
                    break;
                } // end if
    ...
    function parseURL(url)
    {
        ...
        if (_loc2.protocol != undefined && (_loc2.protocol == "rtmp:/" || _loc2.protocol == "rtmpt:/" || _loc2.protocol == "rtmps:/"))
        {
            ...
            if (url.charAt(_loc3) == "?")
            {
                ...
                if (_loc6.server != undefined)
                {
                    _loc2.wrappedURL = _loc2.wrappedURL + "/";
                    _loc2.wrappedURL = _loc2.wrappedURL + _loc6.server;
                } // end if
